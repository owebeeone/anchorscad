...
class VesaAdapter(ad.CompositeShape):
    ...
    outer_screw_shaft_overall_length: float = ad.dtfield(self_default=lambda s: s.t)
    outer_screw_shaft_thru_length: float = ad.dtfield(self_default=lambda s: s.t)
    ...
    outer_screw_hole_node: ad.Node = ad.ShapeNode(CountersunkScrew, prefix='outer_screw_')
    screw_cage_size: tuple = ad.dtfield(
        self_default=lambda s: (s.outer_vesa_centres, s.outer_vesa_centres, s.t)
    )
    ...
    def build(self) -> ad.Maker:
        ...
        for i in range(4):
            maker.add_at(
                screw_shape.composite(('outer_screw', i)).at('top'),
                'screw_cage',
                'face_corner',
                'top',
                i,
