...
def find_grad_multiple(target_size: np.array, allowed_multiples_log10):
    ...
    for am in sorted(allowed_multiples_log10, reverse=True):
        ...
        selected = np.where(possible >= (targets_log10 - GRAD_EPSILON / 2), possible, selected)
...
class SvgGraduationRenderer(object):
    ...
    def render(self):
        ...
        grad_size, elems, divider = find_grad_multiple(grad_sizes, self.grad_multiples_log10)
        ...
        def add_grad(grads):
            grads.append(
                f'<path d="M {sp[0]:G} {sp[1]:G} L {ep[0]:G} {ep[1]:G}" stroke="{lc}" stroke-width="{w[0]}"/>')
...
class SvgRenderer(object):
    ...
    path_render_node: dt.Node=dt.dtfield(dt.Node(SvgPathRenderer), init=False)
    path_render: SvgPathRenderer=dt.dtfield(self_default=lambda s: s.path_render_node())
    ...
    def __post_init__(self):
        ...
        self.margin_top_left_px = l.GVector(LIST_3_FLOAT_0([self.img_margin_size,] * 2))
        self.margin_bot_right_px = l.GVector(LIST_3_FLOAT_0(self.target_image_size)) - self.margin_top_left_px
        self.margin_top_left_ms = inv_xform * self.margin_top_left_px
        self.margin_bot_right_ms = inv_xform * self.margin_bot_right_px
        
        self.grad_top_left_px = l.GVector(LIST_3_FLOAT_0([self.grad_margin_size,] * 2))
        self.grad_bot_right_px = l.GVector(LIST_3_FLOAT_0(self.target_image_size)) - self.grad_top_left_px
    ...
    def to_svg_string(self):
        ...
        seq = (hdr[0], g[0], *self.get_frame(), *rgs, *p, *gs, g[1], hdr[1], '')
        return '\n'.join(seq)
