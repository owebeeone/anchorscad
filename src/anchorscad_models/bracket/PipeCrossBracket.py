...
from anchorscad_models.screws.CountersunkScrew import CountersunkScrew, FlatSunkScrew


@ad.shape
@ad.datatree
class PipeCrossBracket(ad.CompositeShape):
    '''
    Pipe cross bracket. Holds 2 PVC pipes together using a ziptie.
    '''
    ...
    EXAMPLES_EXTENDED={'small': ad.ExampleParams(
                       ...
                       'large': ad.ExampleParams(
                            ad.args(radius2=24.6 / 2,
                                      padding=0.25)),  
                       'alternate': ad.ExampleParams(
                             ad.args(radius2=19.31 / 2,
                                       padding=0.5,
                                       tie_edge_offset=6,
    ...
    NOEXAMPLE_ANCHORS=(
                ad.surface_args('bbox', 'face_corner', 0, 0),
                ad.surface_args('base', 'face_corner', 3, 1),)
    ...
    def build(self) -> ad.Maker:
        ...
        maker = ad.Box(dimens).cage('bbox').at(
            'face_corner', 0, 0)
        
        base_bbox = ad.Box([size_x, self.screw_depth, size_z])
        
        base_type = (ad.ModeShapeFrame.SOLID 
        ...
        maker.add_at(base_bbox.cage('base_insert').at('face_centre', 0),
                     'base', 'face_centre', 0, post=ad.rotZ(-90) * ad.rotY(180))
        ...
        shape = self.linear_extrude_node(path, dimens[2])
        shape_type = (ad.ModeShapeFrame.CAGE 
                      if self.gen_alternate_shape 
                      else ad.ModeShapeFrame.SOLID)
        
        maker.add_at(shape.named_shape('bracket', shape_type).at('edge0', 1.0),
            'face_corner', 0, 0, post=ad.rotY(180))
        ...
        if self.gen_alternate_shape:
            ...
            alt_shape = ad.Box(alt_size)
            maker.add_at(alt_shape.solid('alt_box').at('face_edge', 1, 1),
                         'base', 'face_edge', 3, 2, post=ad.ROTY_180)
            
            alt_hole_cage = ad.Box(
                [alt_shape.size[1], 
                 alt_shape.size[1], 
            ...
            alt_hole = ad.Cone(h=alt_hole_cage.size[2] + 0.01, 
        ...
        tag_shape = self.linear_extrude_node(tag_path, tag_extrude_height)
        
        # Wedge for reinforcement of tag
        wedge_path = (ad.PathBuilder()
            .move([0, 0])
        ...
        wedge_shape = self.linear_extrude_node(wedge_path, self.clip_thickness)
        
        tag_solid_maker = tag_shape.solid('tag').at('edge0', 0)
        
        tag_solid_maker.add_at(wedge_shape.solid('wedge').at('edge0', 0.0),
                 'edge9', 0, 
                 post=(ad.rotY(180) * ad.rotZ(180) * ad.rotX(90) 
                       * ad.translate([0, -self.clip_thickness, tag_shape.h])))  
        ...
        at_args1 = ad.args('nipple_plate', 0.5, post=ad.translate([0, 2, 1]))
        at_args2 = ad.args('nipple_plate', 0.5, post=ad.translate([0, 8, 1]))
        ...
        screw_transform = ad.rotY(180) * ad.translate([0, self.clip_height / 2, 0])
        ...
        maker.add_at(tag_solid_maker.composite('tag').at('edge0', 0),
            'face_corner', 0, 0, post=ad.rotY(-90))
        
        
        maker.add_at(tag_hole_maker.composite('tag_hole').at('edge1', 0),
            'base', 'face_corner', 3, 1, post=ad.rotY(0) )
        ...
        maker.add_at(screw_2_locator_shape.composite('screw2_1').at('face_corner', 0, 0),
                     'base', 'face_corner', 5, 1, post=ad.translate(
                         [0, 0, 0]))
        maker.add_at(screw_2_locator_shape.composite('screw2_2').at('face_corner', 0, 0),
                     'base', 'face_corner', 2, 3, post=ad.translate(
                         [0, 0, 0]))
            ...
            for i, f in enumerate(self.lock_screw_offsets):
                ...
                maker.add_at(lock_screw.composite(
                    f'lock_screw_{i + 1}').at('screw_cage', 'top'),
                             ...
                             post=ad.translate([0, y_offs, 0]) * ad.rotX(180))
        ...
        tie_shape = self.linear_extrude_node(tie_path, self.radius1 * 2)
        ...
        tie_maker_l = tie_shape_type('tie_hole').at(
            ...
            post=ad.translate([-tie_edge_offset, 0, wing_point_y]) * ad.rotX(-45))
        ...
        maker.add_at(tie_maker_r.composite(('tag', 'edge6', 0)).at(), 
                     'tag', 'edge6', 1, 0, post=ad.ROTX_90) 
        
        maker.add_at(tie_maker_r.composite(('tag', 'edge7', 1)).at(), 
                     'tag', 'edge7', 1, post=ad.ROTX_90 * ad.ROTV111_240) 
        maker.add_at(tie_maker_l.composite(('tag', 'edge7', 0)).at(), 
                     'tag', 'edge7', 0, post=ad.ROTX_270 * ad.ROTV111_240) 
